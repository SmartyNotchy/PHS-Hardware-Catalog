...
async function post_cmd(cmd) {
    return fetch(SERVER_URL, {
        method: "POST",
        headers: {'Content-Type': 'application/json', "username": "SMCS_PFP", "password": "ThreeComponentsAhead"}, 
        body: cmd.to_json()
    }).then((response) => response.json()).then((json) => json.body).catch(error => console.error('Error:', error));
}

async function get_cmd(cmd) {
    return fetch(SERVER_URL + `?${new URLSearchParams({ cmdName: cmd.cmdName, uuid: CLIENT_UUID, args: cmd.args })}`)
        .then((response) => response.json()).then((json) => json.body).catch(error => console.error('Error:', error));
}

async function calculate_ping() {
    send_time = new Date();
    return get_cmd(new DataCommand("ping", []))
        .then((json) => Math.round(new Date().getTime() - send_time.getTime()));
}
